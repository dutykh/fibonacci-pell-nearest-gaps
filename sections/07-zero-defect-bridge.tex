...
\begin{proof}
Put $w=\mathsf S^tz$ and $k=d+2t$. The hypotheses ensure that $w$ is
positive and ordered. At a hit, \cref{eq:lorentz-cone} and
$C_k^2-2P_k^2=(-1)^d$ give

\begin{equation}
 \mathcal B(w)^2
 =C_k^2-\bigl(\mathcal Q(z)^2+(-1)^d\bigr)<C_k^2.
 \label{eq:B-below-C}
\end{equation}

Because $w$ is positive and ordered, $0<\mathcal B(w)<C_k$. For
...
$\mathcal B(w)-C_k=-E/C_{2t}$, so $D,E>0$. Since
...
\begin{equation}
 \lambda^{k-1}-A_w^2=g\lambda^{-1}.
 \label{eq:evolved-gap}
\end{equation}

At the hit, $g=C_k-\mathcal B(w)$. Combining this with
\cref{eq:key-seed-identity} at $w$ gives

\begin{equation}
 \lambda A_w^2-g
 =2\mathcal B(w)+(-1)^{k+1}\lambda^{-k}>0.
\end{equation}

For even $k$, positivity follows from
$2\mathcal B(w)\geqslant2>\lambda^{-k}$; for odd $k$ it is immediate.
