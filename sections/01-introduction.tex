...
\cref{sec:two-arms} explains why. Each fixed branch of the Markoff tree carries
a quadratic Pisot unit, the dominant root of $X^2-3m_0X+1$ for the Markoff
number $m_0$ that the branch keeps. Pisot contraction therefore controls the
scale on every branch; it does not single out Fibonacci and Pell. What the
orbit bridge of \cref{sec:zero-defect} consumes is stronger: an integral
square root acting compatibly on the two-square data. Comparing traces shows
that any such half-step forces $3m_0-2$ to be a perfect square. The Fibonacci
and Pell arms, carrying $\varphi$ and $\lambda$, provide the two explicit
actions used here and are the two constant-directive branches. The necessary
square condition is not proved to select them globally, and no such global
classification is used in the paper.
...
\Cref{sec:two-arms} explains the branch Pisot scales and half-step obstruction.
