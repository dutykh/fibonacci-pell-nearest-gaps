...
a norm-$-1$ unit. The two surviving examples already display the two possible
orientations:
...
For $q\geqslant1$, $\sigma\in\{1,-1\}$, and positive $n$, the two conditions
\cref{eq:intro-window,eq:intro-target} hold exactly in the two rows
...
$\min\{h,q\log_\lambda\varphi\}$, and only then can the moving-coefficient
...
$\log_\lambda\sqrt2-\eta=0.3300\ldots$, and that the density statement
...
$N(X+Y\sqrt2)=\pm1$ define the norm-one torus and its norm-$-1$ torsor, so
...
The two involutions of the biquadratic field $\Q(\sqrt2,\sqrt5)$ have
different jobs: one controls Pell norms, while the other separates the
...
present question is different in kind. The window
\cref{eq:intro-window} is only its Archimedean part, and the arithmetic lies
in the exact norm--content equation \cref{eq:intro-target} imposed on the gap
...
unordered pair $\{a,b\}$. The problem has arithmetic, geometric, word-
combinatorial, and trace formulations; see Aigner \cite{Aigner2013}, the
