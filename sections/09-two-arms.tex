...
\subsection{Every branch carries a quadratic Pisot unit}
...
\begin{proposition}[Branch recurrence and its Pisot unit]
...
Both values are Pisot units: their respective conjugates are
$-\varphi^{-1}$ and $-\lambda^{-1}$. This does not contrast them with the
fixed-branch units of \cref{eq:branch-perron}, which are Pisot on every branch.
...
fixed-branch unit $\beta(m_0)$. For a constant directive, the continued-
fraction value equals the Perron eigenvalue of its one-letter
continued-fraction matrix, and its square is $\beta(1)$ or $\beta(2)$. Outside
the constant cases the three objects need not coincide. For example,
...
so the continued-fraction value is not an algebraic integer. Its period
matrix, by contrast, has the Pisot eigenvalue $2+\sqrt3$. For a nonperiodic
directive the continued-fraction value need not even be algebraic. Thus a
Pisot--Perron comparison between branches is meaningful only after the
associated number has been specified. In this paper the relevant branch
object is $\beta(m_0)$, and the exceptional property of the two units in
...
cylinder. If the continuation thereafter follows one fixed branch, the Pisot
estimate of \cref{rem:pisot-branch-scale} controls its periodic scale
...
excluded. The general Pisot branch recurrence alone supplies neither the
