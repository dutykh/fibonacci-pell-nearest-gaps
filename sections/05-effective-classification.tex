...
The proof is at $q\geqslant7$, so $\varphi^{2q}\geqslant\varphi^{14}$. Since
$\varphi>8/5$ one has

\begin{equation}
 \varphi^{14}=377\varphi+233>842,
 \label{eq:phi14}
\end{equation}
...
Normalising \cref{eq:moving-main} and using \cref{eq:A-R-bounds} yields the
logarithmic form
...
The first three conditions are the hypotheses of
\cref{lem:absolute-reduction}; the fourth is the quantitative enclosure that
...
\Cref{eq:convergent-enclosure,eq:fixed-eta,eq:fixed-cutoff}
...
\Cref{eq:convergent-enclosure,eq:moving-eta,eq:moving-cutoff}
prove
...
\cref{eq:convergent-enclosure,eq:fixed-eta,eq:fixed-cutoff,eq:moving-eta,eq:moving-cutoff}
