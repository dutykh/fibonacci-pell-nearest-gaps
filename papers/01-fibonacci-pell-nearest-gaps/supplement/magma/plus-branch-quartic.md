# Recorded computation: complete plus-branch $j=1$ quartic exclusion
...
- **Date:** 2026-08-23
- **Objective:** classify the nearest-gap criterion plus-branch hits whose normalized nearest-gap
  exponent is $j=1$.
- **Status:** complete computer-assisted classification, independently rerun
  with distinct seeds and exact rank checks. The only solutions of
  the resulting recurrence equation are the three low pairs displayed
  below. In the nearest-gap criterion admissible domain the coefficient equation retains
...
Conversely, this note only classifies equation (1); the nearest-gap criterion supplies the
...
The exact source is
../magma/plus_branch_quartic.m. It was submitted to the
...
The standard-library check in
../checks/check_plus_branch_quartic.py confirms the shifts,
...
The independent rerun, the exact Jacobian-rank check and the separate check are
in
../magma/plus_branch_quartic_second.m, and
../checks/check_plus_branch_quartic_second.py.
