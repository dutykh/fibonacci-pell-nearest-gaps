# Recorded independent computation: complete quartic exclusion of the plus $j=1$ branch
...
- **Date:** 2026-08-23
- **Objective:** Classify the exact the nearest-gap criterion plus-branch boundary equation
  $$
  P_M=3F_q^2+2(-1)^q
  $$
...
- **Inputs:** the nearest-gap criterion, the exact boundary identity derived in
  `2026-08-23-nearest-even-gap-recurrence-.md`, the negative Pell
...
If the nearest-gap criterion plus gap has normalized unit exponent $j=1$, its exact
...
I submitted
`../magma/plus_branch_quartic_second.m`
to the University of Sydney's public Magma calculator. This was a run
independent of the 's first calculation. It used the distinct fixed
...
The full transcript of the check-relevant output was
...
the nearest-gap criterion plus branch with normalized gap exponent $j=1$ in the unresolved
range.
