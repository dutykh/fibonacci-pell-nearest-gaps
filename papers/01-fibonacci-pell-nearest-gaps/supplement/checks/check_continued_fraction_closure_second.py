...
"""Second, independent fixed-point computation for the adjusted full-block closure.
...
exact terminal the nearest-gap criterion enumeration.

Run from the supplement directory with

    python3 -B supplement/checks/check_continued_fraction_closure_second.py
"""
...
def exact_gap_enumeration() -> list[tuple[int, int, int]]:
    """Independently enumerate the exact the nearest-gap criterion criterion for q < 90."""
...
def main() -> None:
    ...
    print(
        "PASS: second independent outward-rounded fixed-point computation; common "
        "convergent certified by Legendre; both fixed epsilons give w<48; "
        f"all moving epsilons give q<90 with worst (sigma,h)={worst}; "
        f"exact the nearest-gap criterion hits below 90 are {hits}, so none has q>=7."
    )
