...
"""Certified continued-fraction closure of the the eight-term logarithmic form full block.
...
exact the nearest-gap criterion enumeration for q < 90.
...
Deterministic command from the supplement directory:
    python3 -B supplement/checks/check_continued_fraction_closure.py
"""
...
def build_logarithmic_intervals() -> tuple[
    ...
    # -0.502), which would make an exact-Fraction check unnecessarily
    # expensive.
...
def certify_fixed_reduction(
    ...
    for sigma in (1, -1):
        ...
        # This is the logarithm-free check that the DP threshold is <48.
...
def exact_nearest_gap_enumeration() -> list[tuple[int, int, int]]:
    """Enumerate the nearest-gap criterion targets exactly for 2 <= q < 90 and 3 not dividing q."""
...
def main() -> None:
    ...
    print(
        ...
        f"({minimum_sector},{minimum_h}); exact q<90 the nearest-gap criterion hits={hits}; "
