...
\cref{lem:normalization}. By \cref{prop:parity-selector}, $3\nmid q$ and
...
The middle expression is also the primitive two-square writing of the
Fibonacci-arm Markoff number $F_{2q+3}$. Thus \cref{eq:jminus1} admits the
Markoff-arm interpretation developed in \cref{sec:two-arms}. The proof below
uses only the Fibonacci identity and Alekseyev's intersection theorem.
...
\begin{remark}[Provenance of the near-square step]
\label{rem:quartic-status}
The completeness input here is the published theorem
\cite{AlekseyevTengely2014}; this boundary classification is not new. It can
also be reached directly: the negative-Pell identity converts
\cref{eq:j1-recurrence}, with $y=F_q$ and $s=(-1)^q$, into

\begin{equation}
 C_M^2=18y^4+24sy^2+7,
\end{equation}

and after $y=z+1$ a Magma computation of the integral points of the two
resulting quartics returns exactly the points corresponding to
\cref{eq:j1-solutions}. This agrees with the published classification, on
which the proof relies; the calculation is recorded in the supplementary material.
\end{remark}
