...
Archimedean nearest-neighbor condition with an exact norm--content condition.
...
Computation enters the proof only at the very end; everything that reduces
the problem to a finite domain is unbounded in $q$.
...
logarithms then give $h=O(\log q)$ and $q<10^{32}$; explicit
rational-interval reductions yield $h<192$ and $q<90$, after which the
finitely many remaining pairs are settled by exact recurrence arithmetic. For odd $n$, the selector forces
$3\mid q$. Its factorization has no analogue of $h<n$, so the proof must be
...
form bound $q$. The same explicit reductions give $h<48$ or $q<96$, and
...
factorization, height, or recurrence mechanism used below.
...
question to common values of their generalized analogues
...
The motivation does, however, distinguish the two sequences, and
\cref{sec:two-arms} explains why. Along each branch that keeps one Markoff
entry $m_0$ fixed, the integral unimodular Cohn monodromy has dominant
eigenvalue equal to the quadratic Pisot unit that solves
$X^2-3m_0X+1=0$. This eigenvalue is not, in general, the value of the
corresponding infinite continued fraction. Its conjugate contraction controls
the scale on every such branch; it does not single out Fibonacci and Pell.
What the orbit bridge of \cref{sec:zero-defect} consumes is stronger: an
integral square root acting compatibly on the two-square data. Comparing
traces shows that any such half-step forces $3m_0-2$ to be a perfect square.
The Fibonacci and Pell arms, carrying $\varphi$ and $\lambda$, provide the two
explicit actions used here and are the two constant-directive branches. The
necessary square condition is not proved to select them globally, and no such
global classification is used in the paper.
...
\begin{figure}[htbp]
 ...
 \begin{tikzpicture}[
  ...
  \node[proof,below left=of selector] (even) {even branch:\\rectangle, heights,\\explicit reduction};
  \node[proof,below right=of selector] (odd) {odd branch:\\reordered heights,\\explicit reduction};
...
The paper is organized as follows. \Cref{sec:system} develops the
...
local rank criterion and simultaneous prime-power recurrence.
\Cref{sec:two-arms} explains the Pisot monodromy scales and half-step
obstruction. We close by stating the exact Markoff-facing boundary and further
directions.
