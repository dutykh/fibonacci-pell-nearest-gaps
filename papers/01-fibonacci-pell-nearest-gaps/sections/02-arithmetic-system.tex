...
\subsection{Quadratic integers and primitive-unit normalization}
...
The norm-$-1$ integral fiber is the coset $\lambda T(\Z)$. Hence a target
...
$D_{q,-1}=B_q$. Cassini's identity
$F_{q-1}F_{q+1}-F_q^2=(-1)^q$ factors $k\mp(-1)^q$, and we record the two
factorizations here because both are used twice below: with
$\varepsilon=(-1)^q$,
...
\end{aligned}
\label{eq:cassini-factors}
\end{equation}

Substituting $\varepsilon=F_{q-1}F_{q+1}-F_q^2$ gives
$R_q-\varepsilon=F_{q+1}(2F_{q+1}-F_{q-1})$ and
$R_q+\varepsilon=F_{q-1}(2F_{q+2}+F_{q+1})$.

\begin{lemma}[Uniqueness and unit normalization]
...
reconstruction below. The odd branch has a different factorization and will
...
\label{lem:rectangle}
Let $q\geqslant2$, assume that $n$ is even and that
...
$g=C_M+W=2V$ on the minus branch. Which factor is selected is decided by
the sign of $W$, and $g<C_M$ makes that sign known: $W=C_M-g>0$ on the plus
branch and $W=g-C_M<0$ on the minus branch. Since $V-U=W$ in both cases,
the selected factor is the smaller of $U,V$, and these are distinct, so
