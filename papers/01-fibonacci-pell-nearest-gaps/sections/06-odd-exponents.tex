...
the primitive-rectangle argument. Its content has a different factorization,
...
the unit normalization has the form
...
Both are positive. By \cref{lem:normalization}, there is a unique odd
integer $j$ such that
...
\begin{lemma}[Odd trace and content factorization]
\label{lem:odd-factorization}
...
\begin{equation}
 ...
 \qquad
 g\mid R_q^2+1.
 \label{eq:odd-content-factorization}
\end{equation}
...
\begin{equation}
 L-3fx=(x-f)(2x-f)>0,
 \qquad
 \frac{C_h}{P_h}=\sqrt{2-\frac1{P_h^2}}\geqslant\frac75>\frac43
 \qquad(h\geqslant3\ \text{odd})
\end{equation}

to obtain positivity again. The middle identity is
\cref{eq:pell-norm} at odd $h$, and $P_h\geqslant P_3=5$ gives the stated
bound; the restriction to $h\geqslant3$ is needed, since $3C_1=3<4=4P_1$. Hence $g<C_M$, so
...
factorization in \cref{eq:odd-content-factorization}. Finally, if a prime
$p$ divides $g$, then $p$ is odd and $R_q^2\equiv-1\pmod p$; hence
$p\equiv1\pmod4$.
\end{proof}

The factorization in \cref{lem:odd-factorization} explains why the even
...
and, by \cref{lem:matveev,eq:matveev-prefactor} with the moving height
parameters \cref{eq:moving-height-parameters}, whose bound $B=2q$ is
admissible here by \cref{eq:odd-n-less-2q},
...
The common rational approximation and the two fixed-shift enclosures
\cref{eq:common-convergent,eq:convergent-enclosure,eq:fixed-kappa,eq:fixed-cutoff}
depend only on $\gamma$, $\mu_\sigma$, and $\lambda$, not on the parity of
$n$. The absolute-value reduction in \cref{lem:absolute-reduction}, applied
with
...
The moving bounds \cref{eq:moving-kappa,eq:moving-cutoff} cover both
signs and every odd $3\leqslant h<192$, hence in particular the present
range $3\leqslant h<48$. A second application of
\cref{lem:absolute-reduction}, now with $a=2q$, $b=n$,
$\mu=\nu_{\sigma,h}$, $\mathcal C=27$, $\rho=\varphi^2$, $w=q$ and
$N_0=2\mathbin{\cdot}10^{32}$, gives $q<90$, contradicting
...
same fixed and moving rational-interval bounds reduce the problem to the
finite domain \cref{eq:odd-terminal-domain}.
