...
The proof is at $q\geqslant7$, so $\varphi^{2q}\geqslant\varphi^{14}$. Adding
the identity $\varphi^{14}=377\varphi+233$ to its conjugate gives
$\varphi^{14}+\varphi^{-14}=377+466=843$, so that

\begin{equation}
 \varphi^{14}=377\varphi+233=843-\varphi^{-14}>842,
 \label{eq:phi14}
\end{equation}

which is more than the value $420$ needed to pass from
$\abs{e^{\Lambda_1}-1}<20\varphi^{-2q}$ to the displayed logarithmic bound.%
\footnote{The coefficients here are not accidental. The Christoffel word
attached to the Markoff number $F_{15}=610$ has exactly $F_{14}=377$ letters
$a$ and $F_{13}=233$ letters $b$; see \cref{sec:two-arms}.}
Normalizing \cref{eq:moving-main} and using \cref{eq:A-R-bounds} yields the
logarithmic form
...
\begin{equation}
 ...
 \qquad
 B=2q.
 \label{eq:moving-height-parameters}
\end{equation}
...
$N_0\geqslant1$. Suppose that $\abs{Q\gamma-p}<1/2$ and that integers
$a,b$ satisfy

\begin{equation}
 0<\abs{a\gamma-b+\mu}<\mathcal C\rho^{-w},
 \qquad
 1\leqslant a\leqslant N_0,
 \label{eq:reduction-hypothesis}
\end{equation}

where $\mathcal C>0$, $\rho>1$, and $w>0$ are real. Put

\begin{equation}
 \kappa=\dist{Q\mu}-N_0\abs{Q\gamma-p},
 \label{eq:kappa}
\end{equation}
...
\begin{equation}
 w<\frac{\log(\mathcal CQ/\kappa)}{\log\rho}.
 \label{eq:reduction-conclusion}
\end{equation}

The symbols $\mathcal C$, $\rho$, $N_0$ and $\kappa$ are local to this lemma
and to its four applications below: two in \cref{sec:classification} and two
in \cref{sec:odd-exponents}.
\end{lemma}

\begin{proof}
Since $ap-bQ\in\Z$, the triangle inequality on $\R/\Z$ gives
...
\begin{align}
 ...
 &\leqslant
 Q\abs{a\gamma-b+\mu}+N_0\abs{Q\gamma-p}.
\end{align}

The strict hypothesis therefore gives $\kappa<\mathcal CQ\rho^{-w}$, which
is equivalent to \cref{eq:reduction-conclusion}.
\end{proof}
...
\subsection{Explicit reductions of the two exponents}

Recall the phase constant $\gamma$ of \cref{eq:window-binet-phase} and set
...
The same constant governs the two reductions and the density theorem, and it
has an intrinsic meaning: it is the exchange rate between the two extreme
arms of the Markoff tree, in the sense made precise in
\cref{sec:two-arms}.
...
Since $q\geqslant7$, \cref{eq:phi14} gives $\varphi^{2q}>842>150$, so both
delta terms, which are bounded by $(3/2)\varphi^{-2q}$, have absolute value
below $1/100$. The three terms in
...
Exact rational intervals for square roots and logarithms prove
...
\begin{equation}
 ...
 2Q\abs{Q\gamma-p}<1,
 \qquad
 \abs{Q\gamma-p}<\frac{26}{10^{36}}.
 \label{eq:convergent-enclosure}
\end{equation}

The third condition implies the hypothesis $\abs{Q\gamma-p}<1/2$ of
\cref{lem:absolute-reduction}; the first two come with the convergent and
are not needed by it. The fourth is the quantitative enclosure that
the margins below actually consume. It is strictly stronger than the
Legendre bound $\abs{Q\gamma-p}<1/(2Q)$, which by itself would leave
$N_0\abs{Q\gamma-p}<22/1000$ and would not suffice. With the displayed
enclosure,

\begin{equation}
 N_0\abs{Q\gamma-p}<\frac{52}{10^4}.
 \label{eq:convergent-error-scaled}
\end{equation}

\begin{remark}[Where $p/Q$ comes from]
\label{rem:convergent-provenance}
The integers of \cref{eq:common-convergent} are not produced by the proof; they
are supplied to it. They were found by expanding $\gamma$ in continued
fractions and taking the first convergent whose denominator exceeds $6N_0$.
The restriction to convergents is not a matter of taste. The reduction lemma
itself asks only for $\abs{Q\gamma-p}<1/2$; what forces the search into the
convergents is the approximation quality the margins consume, which lies far
beyond the Legendre threshold $\abs{Q\gamma-p}<1/(2Q^2)$, and by Legendre's
theorem the only rationals that good are the convergents of $\gamma$. The size
condition $Q>6N_0$ then keeps the parasitic term $N_0\abs{Q\gamma-p}$ well
below the value $1/2$ that $\dist{Q\mu_\sigma}$ can never exceed. None of this
search enters the proof. Once the two integers are written down they are
treated as given data, and \cref{eq:convergent-enclosure} re-establishes by
exact rational arithmetic every property of them that is used. The search is
heuristic; the verification is exact.
\end{remark}

Only the enclosed error is used below. For both signs, the same enclosure
gives

\begin{equation}
 \dist{Q\mu_\sigma}>\frac{82}{1000},
 \qquad
 \kappa_\sigma
 =\dist{Q\mu_\sigma}-N_0\abs{Q\gamma-p}>\frac7{100},
 \label{eq:fixed-kappa}
\end{equation}

where the second inequality follows from the first together with
\cref{eq:convergent-error-scaled}. The same enclosure gives

\begin{equation}
 \kappa_\sigma(\lambda^2)^{48}>30Q.
...
\begin{equation}
 ...
 \mathcal C=30,\quad \rho=\lambda^2,\quad
 w=\min\{h,q\gamma\},\quad N_0=2\mathbin{\cdot}10^{32}.
\end{equation}

\Cref{eq:convergent-enclosure,eq:fixed-kappa,eq:fixed-cutoff}
...
\begin{equation}
 \dist{Q\nu_{\sigma,h}}>\frac{143}{10^4},
 \qquad
 \kappa_{\sigma,h}
 =\dist{Q\nu_{\sigma,h}}-N_0\abs{Q\gamma-p}>\frac9{1000}
 \label{eq:moving-kappa}
\end{equation}

where again the second inequality follows from the first and
\cref{eq:convergent-error-scaled}, and

\begin{equation}
 \kappa_{\sigma,h}(\varphi^2)^{90}>27Q.
 \label{eq:moving-cutoff}
\end{equation}

The narrowest enclosure occurs at $(\sigma,h)=(1,31)$, where
$\dist{Q\nu_{\sigma,h}}=0.01436\ldots$; it remains strictly above the
displayed margin. This is also the reason the sharper enclosure in
\cref{eq:convergent-enclosure} is displayed: the Legendre bound alone
would make $\kappa_{1,31}$ negative, and the reduction lemma would not
apply.

Apply \cref{lem:absolute-reduction}
for each pair with

\begin{equation}
 a=2q,\quad b=n,\quad \mu=\nu_{\sigma,h},\quad
 \mathcal C=27,\quad \rho=\varphi^2,\quad w=q,\quad N_0=2\mathbin{\cdot}10^{32}.
\end{equation}

\Cref{eq:convergent-enclosure,eq:moving-kappa,eq:moving-cutoff}
prove
...
Square roots are bounded between consecutive rational grid points. Each
comparison is then decided from the endpoints of the enclosing intervals: the
lower endpoint of every $\kappa$ against zero, and the lower endpoint of each
left-hand side against the upper endpoint of its right-hand side. Every
inequality asserted below is therefore strict, and the outward rounding cannot
reverse it. In particular,
\cref{eq:convergent-enclosure,eq:fixed-kappa,eq:fixed-cutoff,eq:moving-kappa,eq:moving-cutoff}
are decided by rational arithmetic alone.
...
candidate at $q=1$; this boundary case is settled analytically, not by
enumeration. The bound \cref{eq:q-90} is strict, so the index $q=90$ is
excluded as well, and every remaining index of this parity branch, for both
signs, lies in the range treated next.
