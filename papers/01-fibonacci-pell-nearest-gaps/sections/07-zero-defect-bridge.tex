...
general seed, then specializes it to the two Fibonacci cores and obtains the
...
occurs exactly in the four rows of \cref{eq:intro-orbit-list}. In
particular every anchor of a hit is odd, and the consecutive-Fibonacci core
$z_{q,1}=(F_{q+1},F_{q+2})$ has no hit for $q\geqslant2$.
\end{theorem}

\begin{proof}
First let $q\geqslant2$. As $\mathcal Q(z_{q,\sigma})=-R_q$,
\cref{eq:cassini-factors} gives

\begin{equation}
 \mathcal Q(z_{q,\sigma})^2-1
 =\bigl(R_q-(-1)^q\bigr)\bigl(R_q+(-1)^q\bigr)
 =F_{q-1}F_{q+1}F_{q+2}F_{q+4}>0,
 \label{eq:core-noncanonical}
\end{equation}

so the seed is noncanonical. Whenever
\cref{thm:general-orbit-gap} is available, its strict window forces
$d\geqslant2$: if $d=1$, it would give
$A_{z_{q,\sigma}}^2<1$, whereas $A_{z_{q,\sigma}}>1$ for
$q\geqslant2$. Hence an odd anchor in those cases has $d\geqslant3$, so
$d-1$ is a positive even exponent. This covers the plus sign at every time
and the minus sign for $t\geqslant1$; the minus sign at $t=0$ is settled
below by a height identity that needs no window.
...
\cref{thm:classification-intro} has normalized direction $\lambda$, whereas
...
same bridge applies. At an odd anchor, the only minus target has normalized
