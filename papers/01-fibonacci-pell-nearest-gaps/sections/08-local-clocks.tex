...
\label{lem:cross-gcd-factorizations}
For $3\nmid q$,
...
\end{aligned}
\label{eq:cross-gcd-factorizations}
\end{equation}
\end{lemma}

\begin{proof}
Put $\varepsilon=(-1)^q$ and
$E=2F_{q+1}P_m-F_qC_m$, and recall the Cassini factorizations
$R_q-\varepsilon=F_{q+1}F_{q+2}$ and $R_q+\varepsilon=F_{q-1}F_{q+4}$ of
\cref{eq:cassini-factors}. Direct reduction gives
...
The factorization replaces one two-coordinate gcd by two aligned recurrence
...
\cref{eq:cross-gcd-factorizations}. Among the progression
...
\cref{eq:cross-gcd-factorizations}. They remain valid on the progression
$q=q_0+Q\ell$.
...
\begin{equation}
 \log_\lambda D_q=q\gamma+\beta+O(\varphi^{-2q}).
 \label{eq:local-rotation}
\end{equation}

The irrationality of $\gamma$ was proved in
\cref{thm:candidate-density}; hence $Q\gamma/M$ is irrational. Weyl
...
The criterion also reaches primes outside that residue class: $z_F(17)=9$ is
odd, so $17$ satisfies condition~(i) although $17\equiv1\pmod8$. Assigning
$7$ to $\gcd(F_{q+1},C_{m(q)})$ and $17$ to $\gcd(F_{q+2},C_{m(q)+1})$, that
is to the two different factors of
\cref{lem:cross-gcd-factorizations}, gives, for every $a,b\geqslant1$,
infinitely many rows with
...
\cref{eq:cross-gcd-factorizations} have opposite parity. If both ranks are
even, no pair can contain $p$. This proves the equivalence.

Now let $p\equiv7\pmod8$ and choose $r$ with $r^2\equiv2\pmod p$. For
$\pi=1+r$, the order of $\pi$ has $2$-part at most $2$. If $e$ is its odd
part, then $\pi^e=\pm1$; since $1-r=-\pi^{-1}$, this gives
$C_e\equiv0\pmod p$. Thus $z_C(p)$ is odd, and the criterion applies.
...
exact recurrence computation of the ranks $z_F(241)=120$ and $z_C(241)=20$
gives the two even ranks displayed in the theorem; the already proved criterion yields
...
This local--global contrast is the organizing principle of the final
