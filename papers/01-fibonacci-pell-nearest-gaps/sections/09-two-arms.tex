...
generalizations that can and cannot be attempted.

Throughout, statements established only on a finite family are identified
as such, and none of them is used as a hypothesis.
...
formulation. The one normalization we need beyond the correspondence itself is
...
$3N_{21}=\operatorname{tr}N$ for the corresponding conjugate; we use it in
...
\emph{Normalization.} Changing $(x,y)$ into $(-x,-y)$ leaves
...
with $k\geqslant0$ and produces the Fibonacci Markoff numbers; the rightmost
consists of the words $ab^k$ and produces the Pell Markoff numbers. On both, the letter
...
of the tree that keeps $m_0$ fixed. Its node words are $w_0^{\,k}v_0$ or
$v_0w_0^{\,k}$ for a fixed $v_0$, according to the side on which it extends:
the Fibonacci arm gives $a^kb$, the Pell arm gives $ab^k$, and only the
order the branch produces is Cohn normalized. Write $m_k$ for the Markoff
number of the $k$-th node and $u_k$ for its characteristic root.
...
\begin{proof}
Write $\mathsf C_k$ for the Cohn matrix of the $k$-th node. Since the Cohn
correspondence is multiplicative on concatenation,
$\mathsf C_k=M_{w_0}^{\,k}M_{v_0}$ on a branch extending to the right and
$\mathsf C_k=M_{v_0}M_{w_0}^{\,k}$ on one extending to the left. We write the
first case out; the second is the mirror image of it throughout.

\emph{Step 1: the matrix recurrence.} The matrix $M_{w_0}$ lies in
$\mathrm{SL}_2(\Z)$ and has trace $3m_0$ by
\cref{eq:cohn-normalization}. Its characteristic polynomial is therefore
$x^2-3m_0x+1$, and Cayley--Hamilton gives
$M_{w_0}^2=3m_0M_{w_0}-I$. Multiplying on the right by
$M_{w_0}^{\,k-1}M_{v_0}$, or in the mirror case on the left by
$M_{v_0}M_{w_0}^{\,k-1}$, yields
...
\emph{Step 2: the two entries we need.} By \cref{eq:cohn-normalization}, the
...
$2$ is exactly the normalization $1\leqslant D\leqslant m$ recorded in
\cref{eq:cohn-normalization}, which holds at every node.
...
$(3m_0-d)(3m_0+d)=4$, whose only positive factorization into factors of the
...
two hypotheses coincide on Markoff numbers: for odd $c$ both $3c-2$ and
$3c+2$ are odd, so each equals its own greatest odd divisor, while for even
$c$ the quotients $(3c-2)/4$ and $(3c+2)/8$ are odd integers
\cite[Lemma~4.2(3)]{Srinivasan2009}.

Srinivasan works in exactly the order met here. Her uniqueness criterion
\cite[Theorem~5.4]{Srinivasan2009} asks, for an odd Markoff number $c$, that
every ambiguous form lying in the principal genus of the form class group of
discriminant $9c^2-4$ correspond to a divisor of $3c-2$, and for a $c$ that is not a prime power she proves that
...
data by one step, and $\mathsf S^2$ realizes
...
Heuristically, the square relation is forced by any such fixed action. If
...
growth rate $\varepsilon^2$, one expects
$\beta(m_0)=\varepsilon^2$. Making this precise would need $\mathsf T$
hyperbolic and $z_0$ off its contracting eigenline, so the relation enters
the theorem below as a hypothesis rather than as a consequence.
...
nothing below depends on the answer. This computation is recorded in the
...
and the rational $p/Q$ of \cref{eq:common-convergent} is an explicit
...
this paper is exact, and is summarized in \cref{tab:dictionary}. Its first row
